...
interface Props {
  contents: JournalModel[];
  language?: string | string[];
}

interface IndexResearchItemProps extends JournalModel {
  delay?: number;
  language: string;
}
...
const IndexJurnal: React.FC<Props> = ({ contents, language }) => (
  <section className="ftco-section penelitian-wrapper">
    ...
    {/* <ScrollAnimation animateIn="fadeInUp" animateOnce delay={120 * 5}>
      <p className="section-index-beranda__arsip">
        <Link
          href="/[language]/list-jurnal-pilihan"
          as={`/${language}/list-jurnal-pilihan`}
        >
          <a>
            
            {getKeyValue(sentences.LihatArsipjurnal_content)(
                selectLanguage(language)
              )}
            {' '}
              &nbsp;
